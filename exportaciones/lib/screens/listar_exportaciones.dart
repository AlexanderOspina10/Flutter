...
Future<List<Exportacion>> fetchPosts() async {
  final response = await http
      .get(Uri.parse('https://exportaciones.onrender.com/exportaciones'));
...
class _ListarExportacionesState extends State<ListarExportaciones> {
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Precio por kilo'),
                  keyboardType: TextInputType.number,
                ),
                TextField(
                  controller: precioDolarController,
                  decoration:
                      const InputDecoration(labelText: 'Precio en dólares'),
                  keyboardType: TextInputType.number,
                ),
              ],
            ),
  ...
  void eliminarExportacion(Exportacion exportacion) async {
    try {
      ...
      String url =
          'https://exportaciones.onrender.com/exportaciones?id=${exportacion.id}';
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'La exportación con ID ${exportacion.id} se eliminó correctamente.');
        setState(() {});
      } else {
